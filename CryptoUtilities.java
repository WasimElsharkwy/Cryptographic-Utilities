...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //if n and m are the same, the GCD is n (or m)
        if (n.equals(m)) {
            keepGoing = false;
        }

        //recursive function to repeatedly call GCD(m, n mod m) until m =0
        if (!m.isZero() && keepGoing) {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        //this is equivalent to n1 % 2
        remainder = n1.divide(TWO);

        //checking if n is zero, zero is even
        if (n.isZero()) {
            even = true;
            //if the remainder is zero, n is divisible by two, so it is even
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //making copies of variables to be manipulated
        ...
        //nBase will be kept at the original value of n to be used if p is odd
        NaturalNumber nBase = n.newInstance();
        nBase.copyFrom(n);

        //this is equivalent to pMod = pCopy % 2
        NaturalNumber pMod = pCopy.divide(TWO);

        //anything raised to 0 = 1
        if (p.isZero()) {
            n.setFromInt(1);
        }
        //recursive fast power code with NN
        else {
            powerMod(n, pCopy, m);
            nCopy.copyFrom(n);
            n.multiply(nCopy);

            //if p is odd, multiply by the original n
            if (!pMod.isZero()) {
                n.multiply(nBase);

            }
            //this code sets n to n % m
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //creating a NN with value of n -1
        NaturalNumber nMinusOne = n.newInstance();
        nMinusOne.copyFrom(n);
        nMinusOne.subtract(ONE);

        //creating two copies of w since powerMod changes the value of its
        //inputs, but w is in restores mode
        ...
        //boolean variable to keep track of witness
        boolean isWitness = false;

        //using powerMod to find w^2 % n and w^(n-1) % n
        //since the contract of this method requires that n > 2, we do not need
        //to check if n > 1 to use the powerMod function
        powerMod(wCopy1, TWO, n);
        powerMod(wCopy2, nMinusOne, n);

        //if either w^2 % n = 1 or w^(n-1) % n =/= 1, then w is a witness
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            boolean validWitness = false;
            boolean isWitness = false;
            int noOfWitnesses = 0;

            //generating 30 numbers to be potential witnesses
            for (int i = 0; i < 30; i++) {
                NaturalNumber w = n.newInstance();

                //loop to ensure that the 30 numbers generated can be witnesses
                while (!validWitness) {
                    w = randomNumber(n);

                    //making sure w fits the requires clause of
                    //isWitnessToCompositeness
                    if (w.compareTo(ONE) > 0 && w.compareTo(nMinusOne) < 0) {
                        isWitness = isWitnessToCompositeness(w, n);
                        if (isWitness) {
                            noOfWitnesses++;
                        }
                        validWitness = true;
